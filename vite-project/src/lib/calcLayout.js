...
export function calcLayout(
	...
	const { cluesFontSize, cluesWidth, cluesHeight, cellSize } = calcCellSize(
		canvasWidth,
		canvasHeight,
		[ maxNumRowClues, numColumns ],
		[ maxNumColumnClues, numRows ]
	);
...
const ROW_CLUE_CELL_ASPECT_RATIO =
	( 1 + 2 * HORIZONTAL_PADDING_RATIO ) * FONT_SIZE_RATIO;
const COLUMN_CLUE_CELL_ASPECT_RATIO =
	( 1 + 2 * VERTICAL_PADDING_RATIO ) * FONT_SIZE_RATIO;

const MAX_CLUE_CELL_WIDTH =
...
function calcCellSize(
...
) {
	{
		...
		if ( fontSize <= MAX_FONT_SIZE ) {
			return {
				...
				cluesWidth:
					ROW_CLUE_CELL_ASPECT_RATIO *
					cellSize *
					numHorizontalCells[ 0 ],
				cluesHeight:
					COLUMN_CLUE_CELL_ASPECT_RATIO *
					cellSize *
					numVerticalCells[ 0 ],
				cellSize,
			};
...

	{
		...
		const gridWidth =
			canvasWidth -
			MAX_CLUE_CELL_WIDTH * numHorizontalCells[ 0 ] -
			2 * MIN_PADDING;
		const gridHeight =
			canvasHeight -
			MAX_CLUE_CELL_HEIGHT * numVerticalCells[ 0 ] -
			2 * MIN_PADDING;
		const cellSize = Math.min(
